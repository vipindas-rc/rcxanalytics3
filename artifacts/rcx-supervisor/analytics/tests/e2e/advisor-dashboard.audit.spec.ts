...
test('dashboard picker keeps project creation below the existing list', async ({ page }) => {
  const workspace = fixture()
  workspace.projects.push({ id: 'project-test', name: 'Test' })
  await routes(page, workspace)
  await openReport(page)
  await page.getByRole('button', { name: 'More chart actions' }).click()
  await page.getByRole('menuitem', { name: 'Add to dashboard' }).click()
  ...
  await expect(dialog.getByText('1. Project')).toHaveCount(0)
  await expect(dialog.getByRole('textbox', { name: 'Search projects' })).toHaveCount(0)
  await expect(dialog.getByText('Sales by region')).toHaveCount(0)
  await expect(dialog.getByText('Choose the project and dashboard that should contain this chart.')).toHaveCount(0)

  const projectList = dialog.locator('.project-picker-list').first()
  const createProject = dialog.getByRole('button', { name: 'Create project' })
  const projectRows = projectList.locator('.project-picker-row')
  const listBox = await projectList.boundingBox()
  const createBox = await createProject.boundingBox()
  const projectRowBoxes = await projectRows.evaluateAll(rows => rows.map(row => {
    const box = row.getBoundingClientRect()
    return { bottom: box.bottom, height: box.height, top: box.top }
  }))
  expect(listBox).not.toBeNull()
  expect(createBox).not.toBeNull()
  expect(createBox!.y).toBeGreaterThan(listBox!.y + listBox!.height)
  expect(projectRowBoxes).toHaveLength(2)
  expect(projectRowBoxes[1].top - projectRowBoxes[0].bottom).toBeGreaterThanOrEqual(6)
  expect(projectRowBoxes[1].top - projectRowBoxes[0].bottom).toBeLessThanOrEqual(10)

  const footerCancel = dialog.locator('.sui-dialog-actions-root').getByRole('button', { name: 'Cancel' })
  const footerAdd = dialog.locator('.sui-dialog-actions-root').getByRole('button', { name: 'Add to dashboard' })
  const footerCancelBox = await footerCancel.boundingBox()
  const footerAddBox = await footerAdd.boundingBox()
  expect(footerCancelBox).not.toBeNull()
  expect(footerAddBox).not.toBeNull()
  expect(Math.abs(footerCancelBox!.y - footerAddBox!.y)).toBeLessThanOrEqual(1)
  expect(Math.abs(footerCancelBox!.height - footerAddBox!.height)).toBeLessThanOrEqual(1)

  await createProject.click()
  await expect(dialog.getByRole('textbox', { name: 'Project name' })).toBeVisible()
  const createPanel = dialog.locator('.dashboard-picker-create-inline')
  const createCancel = createPanel.getByRole('button', { name: 'Cancel' })
  const createConfirm = createPanel.getByRole('button', { name: 'Create project' })
  const createCancelBox = await createCancel.boundingBox()
  const createConfirmBox = await createConfirm.boundingBox()
  expect(createCancelBox).not.toBeNull()
  expect(createConfirmBox).not.toBeNull()
  expect(createCancelBox!.y).toBe(createConfirmBox!.y)
  expect(createCancelBox!.height).toBe(createConfirmBox!.height)
...
test('project and dashboard menus remove only their own chart placement', async ({ page }) => {
  ...
  await page.goto(appPath('/saved'))
  await page.getByRole('button', { name: 'More chart actions' }).click()
  await page.getByRole('menuitem', { name: 'Remove saved chart', exact: true }).click()
  await expect(page.getByTestId('chart-card')).toHaveCount(0)
  await page.goto(appPath('/dashboards/dashboard-team'))
