...
if (live) {
  ...
  Object.assign(options, { invoke: async ({ question, model, maxInputTokens, maxOutputTokens }: any) => {
    ...
    return { output: JSON.parse(text), inputTokens: body.usage?.input_tokens ?? maxInputTokens, outputTokens: body.usage?.output_tokens ?? maxOutputTokens, requestId: body._request_id ?? body.id, modelCalls: 1, toolCalls: 0, repairs: 0 }
  } })
}
