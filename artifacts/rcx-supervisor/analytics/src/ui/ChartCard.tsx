...
import { CHART_CARD_URL_KEYS, useChartCardUrlState, type ChartInspection } from './useChartCardUrlState'
...
export function ChartCard({ artifact, dataset, filters = [], preference, eager, onReady, onAsk: _onAsk, onAdvisor, onSave, onDashboard, onExport, onCopy, onRemove, removeLabel = 'Remove from dashboard', showRendererControl = true, showAdvisor = true }: {
  artifact: Artifact; dataset: Dataset; filters?: Filter[]; preference?: Renderer; eager?: boolean; onReady?: (id: string, error?: string) => void;
  onAsk: (artifact: Artifact) => void; onAdvisor?: (artifact: Artifact, question: string, trigger: HTMLElement) => void; onSave: (artifact: Artifact) => void; onDashboard: (artifact: Artifact) => void; onExport?: (artifact: Artifact, dataset: Dataset, presentation?: Presentation) => void | Promise<void>; onCopy?: (artifact: Artifact, dataset: Dataset, presentation?: Presentation) => void | Promise<void>; onRemove?: (artifact: Artifact) => void; removeLabel?: string; showRendererControl?: boolean; showAdvisor?: boolean;
}) {
  const card = useRef<HTMLDivElement>(null)
  const [portalContainer, setPortalContainer] = useState<HTMLElement | null>(null)
  const setCardNode = useCallback((node: HTMLDivElement | null) => {
    card.current = node
    if (node) setPortalContainer(node.closest<HTMLElement>('.analytics-app') ?? node)
  }, [])
  const chartUrl = useChartCardUrlState(artifact.id)
  const { page, inspector, expanded, filters: urlFilters, renderer: urlRenderer, setPage, setInspector, setExpanded, setRenderer } = chartUrl
  ...
  const requestPresentation = useCallback(async (renderer: Renderer) => {
    ...
    setBusy(renderer); setError(''); setPage(0, { replace: true })
    try {
  ...
  }, [artifact.id, artifact.view.chartType, canonicalFilters, setPage])
  useEffect(() => {
    const desiredRenderer = urlRenderer ?? preference ?? artifact.renderer
  ...
  }, [visible, artifact.id, preference, artifact.renderer, currentRenderer, requestPresentation, filtersKey, urlRenderer])
  ...
  const rows = current?.rows ?? (effectiveFilters.length ? [] : dataset.rows)
  ...
  const exportChart = useCallback(async () => {
    setMenuAnchor(null)
    try {
      if (onExport) await onExport(artifact, dataset, current)
      else download(`${artifact.title.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '').toLowerCase() || 'chart'}.json`, { artifact, dataset, presentation: current })
    } catch (caught) {
      setCopyMessage(caught instanceof Error ? caught.message : 'Unable to export this chart.')
    }
  }, [artifact, current, dataset, onExport])
  const copyChart = useCallback(async () => {
    setMenuAnchor(null)
    try {
      if (onCopy) await onCopy(artifact, dataset, current)
      else {
        if (!navigator.clipboard?.writeText) throw new Error('Copy is unavailable in this browser.')
        const link = new URL(window.location.href)
        link.searchParams.set(CHART_CARD_URL_KEYS.artifact, artifact.id)
        link.searchParams.set(CHART_CARD_URL_KEYS.expanded, artifact.id)
        if (current && current.renderer !== artifact.renderer) link.searchParams.set(CHART_CARD_URL_KEYS.renderer, current.renderer)
        if (effectiveFilters.length) link.searchParams.set(CHART_CARD_URL_KEYS.filters, JSON.stringify({ [artifact.id]: effectiveFilters }))
        await navigator.clipboard.writeText(link.toString())
      }
      setCopyMessage('Chart link copied.')
      window.setTimeout(() => setCopyMessage(''), 2400)
    } catch (caught) {
      setCopyMessage(caught instanceof Error ? caught.message : 'Unable to copy this chart link.')
    }
  }, [artifact, current, dataset, effectiveFilters, onCopy])
  const openInspection = useCallback((value: ChartInspection) => {
    setMenuAnchor(null)
    setInspector(value)
  }, [setInspector])
  return <div ref={setCardNode} className="report-card" data-testid="chart-card" data-artifact-id={artifact.id} data-chart-type={artifact.view.chartType}>
    ...
    <div className="report-heading"><div><Text component="h3">{artifact.title}</Text></div><div className="report-heading-actions"><IconButton symbol={OverflowVerticalMd} title="More chart actions" aria-label="More chart actions" size="medium" color="neutral" onClick={event => setMenuAnchor(event.currentTarget)} /></div></div>
     {!effectiveFilters.length && !!artifact.kpis?.length && <div className="kpi-row">{artifact.kpis.slice(0, 3).map((kpi, i) => <div className="kpi" key={i}><Text className="kpi-label">{kpi.label}</Text><Text className="kpi-value">{formatValue(kpi.value, kpi.unit)}</Text>{kpi.detail && <Text className="kpi-detail">{kpi.detail}</Text>}</div>)}</div>}
     {!!effectiveFilters.length && <Text className="chart-notice">Dashboard filters are applied to this view. KPIs remain available on the unfiltered source chart.</Text>}
    {native && <div className="chart-frame">
     ...
     {error && <div role="alert" className="inline-error"><Text>{error}</Text><Button size="small" color="neutral" onClick={() => { failedAttempts.current.clear(); setFailedRenderers({}); void requestPresentation(urlRenderer ?? preference ?? artifact.renderer) }}>Retry chart</Button></div>}
    {(current?.notice || artifact.notice) && <Text component="p" className="chart-notice">{current?.notice ?? artifact.notice}</Text>}
     {copyMessage && <Text role="status" className="chart-action-status">{copyMessage}</Text>}
    ...
    {dataOpen && table}
     <Menu open={!!menuAnchor} anchorEl={menuAnchor} onClose={() => setMenuAnchor(null)} placement="bottom-end" PopperProps={{ container: portalContainer }}>
      ...
      <MenuDivider />
       <MenuItem onClick={() => void exportChart()}>Export chart data</MenuItem>
       <MenuItem onClick={() => void copyChart()}>Copy chart link</MenuItem>
       <MenuDivider />
       {!showRendererControl && native && (['echarts', 'chartjs', 'plotly'] as Renderer[]).map(renderer => <MenuItem key={renderer} onClick={() => { setMenuAnchor(null); setRenderer(renderer); void requestPresentation(renderer) }}>Render with {rendererNames[renderer]}</MenuItem>)}
      {!showRendererControl && native && <MenuDivider />}
       <MenuItem onClick={() => openInspection('definitions')}>Metric definitions</MenuItem>
       <MenuItem onClick={() => openInspection('provenance')}>Data source</MenuItem>
