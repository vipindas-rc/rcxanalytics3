...
export type ModelContext = { question: string; session: Session; artifact?: Artifact; dataset?: Dataset; dashboard?: Dashboard; sourceContext?: SourceContext; reportContext?: ReportContext; signal?: AbortSignal }
...
export function codexModel(directory: string): Model {
 return async context => {
   // Do not statically bundle the Codex SDK into Supervisor's CommonJS host:
   // its ESM internals rely on import.meta.url. Standalone Codex selection
   // still loads the installed server-only SDK at the inference boundary.
   const moduleName: string = '@openai/codex-sdk'
